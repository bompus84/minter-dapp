...
async function loadInfo() {
  ...
  const price = web3.utils.fromWei(info.mintPrice, 'ether');
...
function setTotalPrice() {
  ...
  const totalPriceWei = BigInt(info.mintPrice) * BigInt(mintInputValue);
...
async function mint() {
  ...
  const value = BigInt(info.mintPrice) * BigInt(amount);
  const publicMintActive = await contract.methods.mintingActive().call();
  const presaleMintActive = await contract.methods.presaleActive().call();
